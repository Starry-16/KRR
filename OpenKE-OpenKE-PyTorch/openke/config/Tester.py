...
class Tester(object):
    ...
    def run_link_prediction(self, type_constrain = False):
        ...
        for index, [data_head, data_tail, data_rel] in enumerate(training_range):
            ...
            score = self.test_one_step(data_rel)
            self.lib.testRel(score.__array_interface__["data"][0], index, type_constrain)
    ...
    def head_query(self, type_constrain = False):  
        self.lib.initTest()
        self.data_loader.set_sampling_mode('link')
        if type_constrain:
            type_constrain = 1
        ...
        for index, [data_head, data_tail, data_rel] in enumerate(training_range):
            score = self.test_one_step(data_head)
            self.lib.testHead(score.__array_interface__["data"][0], index, type_constrain)
        return np.where(score == min(score))

    def tail_query(self, type_constrain = False):  
        self.lib.initTest()
        self.data_loader.set_sampling_mode('link')
        if type_constrain:
            type_constrain = 1
    ...
    def rel_query(self, type_constrain = False):  
        self.lib.initTest()
        self.data_loader.set_sampling_mode('link')
        if type_constrain:
            type_constrain = 1
